...
class Player:
    ...
    def move(self, key_bindings, events, room_data, entities, living_items, room, screen):
        ...
        for movement in started_movements:
            if movement == "up":
                ...
                if self.attack_cooldown == self.max_attack_cooldown:
                    ...
                    for item in drops:
                        if item["name"] != None:
                            living_items.append(LivingItem(item["x"], item["y"], lookup(item["name"])(), [randint(4,10), randint(4,10)]))
