...
class Room:
    def __init__(self, data):
        ...
        for y, row in enumerate(data["squares"]):
            ...
            for x, square in enumerate(row):
                new_row[x] = Square(squares_data[square], default_square, constants.FILE_PATH, constants.SQUARE_SIZE, [self.location[0] + (x * constants.SQUARE_SIZE[0]), self.location[1] + (y * constants.SQUARE_SIZE[1])], [x,y])
    ...
    def draw(self, screen, square_size):
        ...
        for y, row in enumerate(self.layout):
            for x, square in enumerate(row):
                texture = pygame.transform.scale(pygame.image.load(constants.FILE_PATH + f"project_lib/assets/squares/air.png").convert(), square_size)
