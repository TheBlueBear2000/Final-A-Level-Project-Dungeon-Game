...
if __name__ == '__main__':
    ...
    while playing: # in game, between levels
        ...
        #lives = json.load(open(constants.FILE_PATH + f"data/levels/rooms_{level}/player.json", 'r'))["lives"]
        
        room = json.load(open(constants.FILE_PATH + f"data/levels/rooms_{level}/1.json", 'r'))
        ...
        while not dead: # in level, between rooms
            ...
            if dead:
                ...
                player.lives -= 1
                print(f"died, lives: {player.lives}")
                break
