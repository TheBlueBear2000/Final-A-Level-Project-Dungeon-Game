        ...
        for movement in started_movements:
            if movement == "up":
                ...
                if self.attack_cooldown >= self.max_attack_cooldown:
                    ...
                    for item in drops:
                        if item["name"] != None:
                            living_items.append(LivingItem(item["x"], item["y"], lookup(item["name"])(None), [randint(-10,10), randint(-10,10)]))
    ...
    def attack(self, player, entities, room, damage):
        ...
        for entity in effectedEntities:
            if entity.team != self.team:
                if entity.take_damage(damage): # returns true if entity is killed
                    self.kills += 1
